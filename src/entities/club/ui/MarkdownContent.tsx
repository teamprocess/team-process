...
export function MarkdownContent({ content, className }: MarkdownContentProps) {
  return (
    <div className={cn('markdown-content space-y-4 sm:space-y-5', className)}>
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          h1: ({ className: headingClassName, ...props }) => (
            <h1 className={cn('text-3xl font-semibold leading-[1.18] tracking-tight text-foreground sm:text-4xl', headingClassName)} {...props} />
          ),
          h2: ({ className: headingClassName, ...props }) => (
            <h2 className={cn('text-2xl font-semibold leading-[1.2] tracking-tight text-foreground sm:text-3xl', headingClassName)} {...props} />
          ),
          h3: ({ className: headingClassName, ...props }) => (
            <h3 className={cn('text-xl font-semibold leading-[1.25] text-foreground', headingClassName)} {...props} />
          ),
          p: ({ className: paragraphClassName, ...props }) => (
            <p className={cn('text-sm leading-7 text-muted-foreground sm:text-base', paragraphClassName)} {...props} />
          ),
          ul: ({ className: listClassName, ...props }) => (
            <ul
              className={cn('list-disc space-y-3 pl-5 text-sm leading-6 text-muted-foreground sm:text-base', listClassName)}
              {...props}
            />
          ),
          ol: ({ className: listClassName, ...props }) => (
            <ol
              className={cn('list-decimal space-y-3 pl-5 text-sm leading-6 text-muted-foreground sm:text-base', listClassName)}
              {...props}
            />
          ),
          ...
          a: ({ className: linkClassName, ...props }) => (
            <a className={cn('font-medium text-primary underline underline-offset-4', linkClassName)} {...props} />
          ),
          blockquote: ({ className: quoteClassName, ...props }) => (
            <blockquote
              className={cn(
                'border-l-2 border-primary/30 pl-4 text-sm italic text-muted-foreground sm:text-base',
                quoteClassName
              )}
              {...props}
            />
          ...
          td: ({ className: cellClassName, ...props }) => (
            <td className={cn('px-4 py-3 align-top text-muted-foreground', cellClassName)} {...props} />
          ),
          code: ({
